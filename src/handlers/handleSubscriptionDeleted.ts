...
export const handleSubscriptionDeleted = async (data: Stripe.Subscription) => {
  try {
    ...
    const dbSub = await Subscription.findOne({ subscriptionId: subscription.id });
    if (!dbSub) throw new ApiError(StatusCodes.NOT_FOUND, 'Subscription not found');

    dbSub.status = 'cancel';
    await dbSub.save();

    // Update user access
    await User.findByIdAndUpdate(dbSub.user, { role: 'USER', hasAccess: false, isSubscribed: false });
