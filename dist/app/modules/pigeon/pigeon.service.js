...
const breeder_model_1 = require("../breeder/breeder.model");
...
const createPigeonToDB = (data, files, user) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    // 🔹 API level trimming
    if (parsedData.ringNumber)
        parsedData.ringNumber = parsedData.ringNumber.trim();
    if (parsedData.name)
        parsedData.name = parsedData.name.trim();
    if (parsedData.fatherRingId)
        parsedData.fatherRingId = parsedData.fatherRingId.trim();
    if (parsedData.motherRingId)
        parsedData.motherRingId = parsedData.motherRingId.trim();
    ...
    if (parsedData.fatherRingId && parsedData.fatherRingId.trim() !== "") {
        let father = yield pigeon_model_1.Pigeon.findOne({ ringNumber: parsedData.fatherRingId });
        ...
        if (father) {
            ...
            father = yield pigeon_model_1.Pigeon.create({ ringNumber: parsedData.fatherRingId, verified: false, user: user._id });
        }
    ...
    if (parsedData.motherRingId && parsedData.motherRingId.trim() !== "") {
        let mother = yield pigeon_model_1.Pigeon.findOne({ ringNumber: parsedData.motherRingId });
        if (mother) {
            // যদি পিজন DB-তে থাকে, তার gender check করা
            ...
            // DB-তে না থাকলে নতুন পিজন create করা
            mother = yield pigeon_model_1.Pigeon.create({ ringNumber: parsedData.motherRingId, verified: false, user: user._id });
        }
        parsedData.motherRingId = mother._id;
    }
    else {
    ...
    if (parsedData.breeder) {
        ...
        if (parsedData.breeder !== "") {
            ...
            let existingBreeder = yield breeder_model_1.Breeder.findOne({
            ...
            if (existingBreeder) {
                ...
                const newBreeder = yield breeder_model_1.Breeder.create({
                    loftName: parsedData.breeder,
                    breederName: parsedData.breeder,
                    status: false,
                    experience: "none",
    ...
    const verifiedExist = yield pigeon_model_1.Pigeon.findOne({
        $or: [
    ...
    if (verifiedExist) {
        ...
        const nameExist = yield pigeon_model_1.Pigeon.findOne({ name: parsedData.name, verified: true });
        if (nameExist)
            throw new ApiErrors_1.default(http_status_codes_1.StatusCodes.CONFLICT, "This name belongs to a verified pigeon and cannot be used");
        // console.log("ring number", ringExist);
    }
    ...
    parsedData.photos = [];
    if (files && files[extraPhotosField]) {
        files[extraPhotosField].forEach((file) => parsedData.photos.push(`/images/${file.filename}`));
    }
    // Optional results
    if (!parsedData.results)
        parsedData.results = [];
    ...
    if (parsedData.ringNumber && parsedData.country && parsedData.birthYear) {
        ...
        if (duplicatePigeon) {
            throw new ApiErrors_1.default(http_status_codes_1.StatusCodes.CONFLICT, "This pigeon is already registered in our database. To add it to your loft database, go to the Pigeon Database and press the “+” button.");
        }
    }
    // Save pigeon
    parsedData.user = user._id;
    const result = yield pigeon_model_1.Pigeon.create(parsedData);
    // Notifications
    ...
    if (["USER", "PAIDUSER"].includes(user.role)) {
        ...
        const users = yield user_model_1.User.find({ role: { $in: ["USER", "PAIDUSER"] } }).select("_id");
        const receiverIds = users.map(u => u._id);
        if (receiverIds.length > 0) {
            yield notification_service_1.NotificationService.createNotificationToDB({
                text: `Pigeon added by ${sender.name}`,
...
const updatePigeonToDB = (pigeonId, data, files, user) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    // Parse incoming data
    const parsedData = typeof data === "string" ? JSON.parse(data) : Object.assign({}, data);
    // Free user restrictions
    if (!["ADMIN", "SUPER_ADMIN"].includes(user.role)) {
        parsedData.verified = false;
        parsedData.iconic = false;
    ...
    if (parsedData.fatherRingId !== undefined) {
        ...
        if (newFatherRing === pigeon.ringNumber) {
            throw new ApiErrors_1.default(http_status_codes_1.StatusCodes.BAD_REQUEST, "You cannot assign the pigeon itself as father.");
        }
        if (newFatherRing) {
            let father = yield pigeon_model_1.Pigeon.findOne({ ringNumber: newFatherRing });
            if (!father) {
                father = yield pigeon_model_1.Pigeon.create({ ringNumber: newFatherRing, verified: false, user: user._id });
            }
            parsedData.fatherRingId = father._id;
        }
        else {
    ...
    if (parsedData.motherRingId !== undefined) {
        ...
        if (newMotherRing) {
            let mother = yield pigeon_model_1.Pigeon.findOne({ ringNumber: newMotherRing });
            if (!mother) {
                mother = yield pigeon_model_1.Pigeon.create({ ringNumber: newMotherRing, verified: false, user: user._id });
            }
            parsedData.motherRingId = mother._id;
        }
        else {
    ...
    // Duplicate check: same ringNumber + country + birthYear
    if (parsedData.ringNumber && parsedData.country && parsedData.birthYear) {
        const originalUnchanged = pigeon.ringNumber === parsedData.ringNumber &&
    ...
    photoFields.forEach(field => {
        if (files && files[field] && files[field][0])
            ...
            parsedData[field] = pigeon[field]; // আগের value রাখো
    });
    // Extra photos
    let updatedPhotos = pigeon.photos || [];
    const remainingPhotos = parsedData.remaining || [];
    if (remainingPhotos.length > 0)
        updatedPhotos = updatedPhotos.filter(photo => remainingPhotos.includes(photo));
    if (files && files.photos)
        updatedPhotos.push(...files.photos.map((file) => `/images/${file.filename}`));
    ...
    // Update database
    const updatedPigeon = yield pigeon_model_1.Pigeon.findByIdAndUpdate(pigeonId, payload, { new: true });
    if (!updatedPigeon)
...
const getAllPigeonsAdminsFromDB = (query) => __awaiter(void 0, void 0, void 0, function* () {
    let baseQuery = pigeon_model_1.Pigeon.find({ status: { $ne: "Deleted" },
        name: { $exists: true, $ne: "" }
    });
    const qb = new queryBuilder_1.default(baseQuery, query);
...
const getMyAllPigeonDetailsFromDB = (userId, query) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (query.birthYear)
        ownFilter.birthYear = Number(query.birthYear);
    if (query.gender)
        ownFilter.gender = new RegExp(`^${query.gender.trim()}$`, "i");
    if (query.country)
        ownFilter.country = new RegExp(`^${query.country.trim()}$`, "i");
    if (query.status)
        ownFilter.status = new RegExp(`^${query.status.trim()}$`, "i");
    if (query.catagory)
        ownFilter.catagory = new RegExp(`^${query.catagory.trim()}$`, "i");
    if (query.breeder) {
        const breederDoc = yield breeder_model_1.Breeder.findOne({ breederName: new RegExp(`^${query.breeder.trim()}$`, "i") });
        if (breederDoc)
            ownFilter.breeder = breederDoc._id;
    }
    const ownPigeons = yield pigeon_model_1.Pigeon.find(ownFilter)
    ...
    const loftPigeonIds = yield loft_model_1.UserLoft.find({ user: userId }).distinct("pigeon");
    const loftFilter = {
        _id: { $in: loftPigeonIds },
        status: { $ne: "Deleted" },
    };
    if (query.birthYear)
        loftFilter.birthYear = Number(query.birthYear);
    if (query.gender)
        loftFilter.gender = new RegExp(`^${query.gender.trim()}$`, "i");
    if (query.country)
        loftFilter.country = new RegExp(`^${query.country.trim()}$`, "i");
    if (query.status)
        loftFilter.status = new RegExp(`^${query.status.trim()}$`, "i");
    if (query.catagory)
        loftFilter.catagory = new RegExp(`^${query.catagory.trim()}$`, "i");
    if (query.breeder) {
        const breederDoc = yield breeder_model_1.Breeder.findOne({ breederName: new RegExp(`^${query.breeder.trim()}$`, "i") });
        if (breederDoc)
            loftFilter.breeder = breederDoc._id;
    }
    let loftPigeons = yield pigeon_model_1.Pigeon.find(loftFilter)
        .populate("user", "name email")
        .populate("fatherRingId", "ringNumber name")
        .populate("motherRingId", "ringNumber name")
        .populate("breeder", "loftName breederName status country")
    ...
    const loftMap = yield loft_model_1.UserLoft.find({ user: userId, pigeon: { $in: loftPigeonIds } })
        .select("pigeon addedAt")
        .lean();
    loftPigeons = loftPigeons.map(p => {
        const loftInfo = loftMap.find(l => l.pigeon.toString() === p._id.toString());
        return Object.assign(Object.assign({}, p), { loftInfo: loftInfo ? { addedAt: loftInfo.addedAt } : undefined });
    });
    // --- Step 3: Combine own + loft pigeons ---
    let allPigeons = [...ownPigeons, ...loftPigeons];
    // --- Step 4: Apply QueryBuilder filters/sorting if any ---
    const qb = new queryBuilder_1.default(pigeon_model_1.Pigeon.find({ _id: { $in: allPigeons.map(p => p._id) } }), query);
    qb.search(["ringNumber", "name", "country"]).filter().sort().fields().populate(["user", "fatherRingId", "motherRingId", "breeder"], {
        user: "name email",
        fatherRingId: "ringNumber name",
        motherRingId: "ringNumber name",
        breeder: "loftName breederName status country",
    });
    let filteredPigeons = yield qb.modelQuery;
...
const deletePigeonFromDB = (id, user) => __awaiter(void 0, void 0, void 0, function* () {
    if (!mongoose_1.default.Types.ObjectId.isValid(id)) {
        throw new ApiErrors_1.default(http_status_codes_1.StatusCodes.BAD_REQUEST, "Invalid ID");
    }
    const pigeon = yield pigeon_model_1.Pigeon.findById(id);
    if (!pigeon) {
        throw new ApiErrors_1.default(http_status_codes_1.StatusCodes.NOT_FOUND, "Pigeon not found");
    }
    if (["USER", "PAIDUSER"].includes(user.role)) {
        // Check if pigeon is in UserLoft of current user
        const userLoftEntry = yield loft_model_1.UserLoft.findOne({ user: user._id, pigeon: id });
        const loftEntry = yield verifyloft_model_1.Loft.findOne({ _id: id, breeder: user._id });
        if (userLoftEntry) {
            yield loft_model_1.UserLoft.deleteOne({ _id: userLoftEntry._id });
        }
        if (loftEntry) {
            yield verifyloft_model_1.Loft.deleteOne({ _id: loftEntry._id });
        }
        ...
        if (!userLoftEntry && !loftEntry) {
            if (!pigeon.user.equals(user._id)) {
                throw new ApiErrors_1.default(http_status_codes_1.StatusCodes.FORBIDDEN, "You can only delete your own pigeons");
            }
            yield pigeon_model_1.Pigeon.findByIdAndDelete(id);
    ...
    yield pigeon_model_1.Pigeon.findByIdAndDelete(id);
    yield loft_model_1.UserLoft.deleteMany({ pigeon: id });
    yield verifyloft_model_1.Loft.deleteMany({ _id: id });
...
const getSiblings = (pigeonId) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!pigeon)
        throw new ApiErrors_1.default(http_status_codes_1.StatusCodes.NOT_FOUND, "Pigeon not found");
    const fatherId = (_b = (_a = pigeon.fatherRingId) === null || _a === void 0 ? void 0 : _a._id) === null || _b === void 0 ? void 0 : _b.toString();
    const motherId = (_d = (_c = pigeon.motherRingId) === null || _c === void 0 ? void 0 : _c._id) === null || _d === void 0 ? void 0 : _d.toString();
    // ২. Full siblings (একই বাবা & একই মা)
    let fullSiblings = [];
    if (fatherId && motherId) {
        fullSiblings = yield pigeon_model_1.Pigeon.find({
            _id: { $ne: pigeon._id },
            fatherRingId: fatherId,
            motherRingId: motherId,
        })
    ...
    if (fatherId && motherId) {
        halfOrConditions.push({ fatherRingId: fatherId, motherRingId: { $ne: motherId } }, { motherRingId: motherId, fatherRingId: { $ne: fatherId } });
    }
    else if (fatherId) {
    ...
    if (halfOrConditions.length > 0) {
        halfSiblings = yield pigeon_model_1.Pigeon.find({
            _id: { $ne: pigeon._id },
            $or: halfOrConditions,
        })
            .populate("fatherRingId")
    ...
    // ৪. Type assign করা
    const siblings = [
        ...fullSiblings.map((s) => (Object.assign(Object.assign({}, s), { type: "Full Sibling" }))),
        ...halfSiblings.map((s) => {
            var _a, _b, _c, _d;
            let halfType = "Half Sibling";
            console.log("Checking half sibling:", s.ringNumber);
            if (fatherId && ((_b = (_a = s.fatherRingId) === null || _a === void 0 ? void 0 : _a._id) === null || _b === void 0 ? void 0 : _b.toString()) === fatherId) {
                halfType = "Half Sibling (same father)";
                console.log("Half Sibling (same father) found:", s.ringNumber);
            }
            else if (motherId && ((_d = (_c = s.motherRingId) === null || _c === void 0 ? void 0 : _c._id) === null || _d === void 0 ? void 0 : _d.toString()) === motherId) {
                halfType = "Half Sibling (same mother)";
                console.log("Half Sibling (same mother) found:", s.ringNumber);
            }
            return Object.assign(Object.assign({}, s), { type: halfType });
        }),
    ];
    return siblings;
});
